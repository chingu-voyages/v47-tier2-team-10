...
export default function Category(props: Props) {
  ...
  return (
    <>
      ...
      >
        <div
          className={`hs-accordion-toggle w-full text-start flex items-center gap-x-3.5 py-2 px-2.5 hs-accordion-active:hs-accordion-active:hover:bg-transparent text-sm text-slate-700 rounded-lg hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-900 dark:text-slate-400 dark:hover:text-slate-300 dark:hs-accordion-active:text-white dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600`}
        >
          <button
          ...
          >
            <MdExpandMore
              className={`${
                isActivityVisible
                  ? "rotate-180 font-bold text-[#2d8630] dark:text-white "
                  : "font-bold text-gray-800 dark:text-gray-400"
              } transform transition duration-200 ease-out `}
            />
            <p
              className={
                isActivityVisible ? "text-[#2d8630] dark:text-white" : ""
              }
            >
          ...
          <div className="ml-auto">
            <div className="flex gap-1 ml-auto">
              <button
                ...
                className={`${
                  isCategoryIconsVisible ? "block" : "lg:hidden block"
                } text-md  text-gray-900 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300`}
              >
                <MdOutlineEdit />
              </button>
              <button
      ...
      <div
        ...
        className={`hs-accordion-content w-full overflow-hidden transition-[height] duration-300 ${
          isActivityVisible ? "" : "hidden"
        }`}
      >
        <ul className="pt-0 ps-2">{activityEl}</ul>
      </div>
